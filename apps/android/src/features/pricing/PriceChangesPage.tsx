...
export function PriceChangesPage(): React.ReactElement {
  ...
  const handleRequest = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!productId.trim() || Number(newPrice) <= 0) return;
    try {
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      push({ type: 'success', msg: 'Approved' });
    } catch (err) {
      push({ type: 'error', msg: String(err) });
    }
  };
  ...
  const handleReject = async (id: string) => {
    try {
      ...
      push({ type: 'success', msg: 'Rejected' });
    } catch (err) {
      push({ type: 'error', msg: String(err) });
    }
  };
    ...
    <div className="page">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Request and approve price changes.</p>
        </div>
        <button type="button" className="btn btn-primary" onClick={() => setIsRequestOpen(true)}>
      ...
      {pendingApproval.length > 0 && (
        <>
          <h2 className="section-title" style={{ marginBlockStart: 'var(--space-4)' }}>Pending ({pendingApproval.length})</h2>
          <div className="table-container" style={{ marginBlockStart: 'var(--space-2)' }}>
            <table className="table">
              ...
              <tbody>
                {pendingApproval.map((pc: { id: string; productId: string; oldPricePiasters: number; newPricePiasters: number }) => (
                  <tr key={pc.id}>
                    <td>{pc.productId}</td>
      ...
      {status === 'loading' && items.length === 0 ? (
        ...
        <div className="table-container" style={{ marginBlockStart: 'var(--space-2)' }}>
          <table className="table">
            ...
            <tbody>
              {items.map((pc: { id: string; productId: string; oldPricePiasters: number; newPricePiasters: number; status: string }) => (
                <tr key={pc.id}>
                  <td>{pc.productId}</td>
                  <td className="table-numeric">{(pc.oldPricePiasters / 100).toFixed(2)}</td>
                  <td className="table-numeric">{(pc.newPricePiasters / 100).toFixed(2)}</td>
      ...
      {isRequestOpen && (
        <Modal open={isRequestOpen} onClose={() => setIsRequestOpen(false)} title="Request Price Change" footer={
          <>
            ...
            <button type="submit" form="pc-form" className="btn btn-primary">Submit</button>
          </>
        }>
          <form id="pc-form" onSubmit={handleRequest} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
