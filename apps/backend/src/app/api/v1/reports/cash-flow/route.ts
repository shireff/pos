...
import { CashFlowScaffold, CashFlowInput } from '@packages/application-reports';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const report = new CashFlowScaffold(repos.paymentTransactionRepo);
    const result = await report.execute({ companyId, from, to } as CashFlowInput);
