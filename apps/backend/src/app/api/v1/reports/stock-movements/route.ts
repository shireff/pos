...
import { StockMovementReport, StockMovementInput } from '@packages/application-reports';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const report = new StockMovementReport(repos.stockMovementRepo);
    const result = await report.execute({ companyId, warehouseId, from, to } as StockMovementInput);
