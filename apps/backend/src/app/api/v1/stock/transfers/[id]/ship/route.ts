...
import { ShipTransferCommand } from '@packages/application-inventory';
import {
...
export async function POST(
  ...
  try {
    ...
    const command = new ShipTransferCommand(transferRepo, movementRepo, stockItemRepo);
    const result = await command.execute({
      companyId: body.companyId ?? 'company-1',
      transferId: id,
      lineShippedQuantities: body.lineShippedQuantities,
    });
