...
import { SubmitTransferForApprovalCommand } from '@packages/application-inventory';
...
export async function POST(
  ...
  try {
    ...
    const command = new SubmitTransferForApprovalCommand(transferRepo);
    const result = await command.execute({ companyId: 'company-1', transferId: id });
