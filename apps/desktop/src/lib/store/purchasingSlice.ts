...
export const updatePurchaseOrder = createAsyncThunk<
  PurchaseOrder,
  {
    ...
    expectedDeliveryDate?: string;
    notes?: string | null;
    lines?: Array<{ id: string; orderedQuantity: number; unitPricePiasters: number }>;
  },
  { rejectValue: string }
>('purchasing/updateOrder', async (input, thunkAPI) => {
  try {
    const endpoint = buildEndpoint(ApiEndpoints.PurchaseOrderById, { id: input.id });
    ...
    return thunkAPI.rejectWithValue(extractMessage(error, 'Failed to update purchase order'));
  }
});
