...
- **BR-AI-007:** Usage is strictly whitelisted for NaraRouter endpoint with allowed models (`kimi-k2.7-code-free`, `mistral-large`, and `mistral-medium-3-5`) within a 5 million token budget. If a primary model fails or times out, the system falls back among the whitelisted models or to graceful degradation, and every response tags its `source` (e.g. `nara_router:kimi-k2.7-code-free`, `nara_router:mistral-large`, etc.) (AI.md §1).
