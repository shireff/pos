...
- **Provider-fallback test:** simulates NaraRouter models failing/timing out and asserts the fallback chain (e.g. falling back among the whitelisted models kimi-k2.7-code-free, mistral-large, and mistral-medium-3-5, then to graceful degradation) behaves per AI.md §1, including correct `source` tagging in the response.
