...
async function main() {
  ...
  try {
    ...
    console.error('ERROR errInfo', JSON.stringify(err.errInfo, null, 2));
    if (err?.errInfo?.details) console.error('ERROR details', JSON.stringify(err.errInfo.details, null, 2));
  } finally {
    await client.close();
  }
}
